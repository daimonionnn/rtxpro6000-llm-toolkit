#!/usr/bin/env bash
# Profile sglang-bf16: Qwen3.8-27B, original BF16 weights, on SGLang, one RTX PRO 6000
# Blackwell (96 GB).
#
#   MODEL_DIR=/models/Qwen3.8-27B ./serve-bf16.sh
#   ./stop.sh
#
# Checkpoint Qwen/Qwen3.8-27B: a dense 27B hybrid (qwen3_5 architecture — Gated
...
#
# Settings follow a published single-card BF16 setup of this model on an RTX PRO
...
print(f"\nQwen3.8-27B — {os.path.basename(os.path.normpath(d))}  ·  engine sglang")
...
docker run -d --name "$NAME" --restart unless-stopped --gpus '"device=0"' \
  ...
  --label "rtxpro6000-llm.profile=qwen3.8-27b-sglang-bf16" \
...
for _ in $(seq 1 120); do
  if docker logs "$NAME" 2>&1 | grep -q "The server is fired up"; then
    echo "ready on http://127.0.0.1:${PORT}  ·  Qwen3.8-27B BF16, SGLang, NEXTN $SPEC_STEPS"; exit 0
  fi
