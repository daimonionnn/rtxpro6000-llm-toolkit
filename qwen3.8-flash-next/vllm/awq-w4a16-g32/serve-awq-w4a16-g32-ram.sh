...
# AWQ). Same layout as the vllm-awq-w4a16 profile's checkpoint, finer quantization:
#
#   routed experts   INT4 weight-only, group 32, asymmetric (zero points), MSE
#                    observer — vs group 128 symmetric; calibrated on STEM and
#                    agentic data in 10 languages; activations stay BF16
#   everything else  BF16, including shared experts, linear attention, router
#                    and the 51B PLE n-gram table (95.4 GiB)
#
# The finer groups cost ~0.6 bits per expert weight, so ~80 GiB of weights go to
# the GPU and the KV pool is smaller than vllm-awq-w4a16's. The PLE table is held
# in host RAM by vLLM's PLE offload worker (VLLM_PLE_CPU_OFFLOAD=1), which needs
# ~96 GiB of free RAM.
#
...
docker run -d --name "$NAME" --restart unless-stopped --gpus '"device=0"' \
  ...
  --ulimit memlock=-1 \
  --label "rtxpro6000-llm.profile=qwen3.8-flash-next-vllm-awq-w4a16-g32" \
  --label "rtxpro6000-llm.quant.experts=INT4-W4A16-g32-asym" \
...
echo "waiting for the server (first start compiles and loads 175 GiB — allow 10-20 min)"
for _ in $(seq 1 180); do
  if curl -sf --max-time 3 "http://127.0.0.1:${PORT}/health" >/dev/null 2>&1; then
    echo "ready on http://127.0.0.1:${PORT}  ·  vLLM, AWQ W4A16 g32 experts, BF16 rest, PLE BF16 in RAM, KV $KV_DTYPE"; exit 0
  fi
