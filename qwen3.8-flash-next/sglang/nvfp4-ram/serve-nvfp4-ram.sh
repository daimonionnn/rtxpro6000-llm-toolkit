#!/usr/bin/env bash
# Profile sglang-nvfp4-ram: SGLang, NVFP4, PLE table in RAM, one RTX PRO 6000 Blackwell (96 GB).
#
# Uses the Docker image built by ../build-local-image/build.sh.
#
#   MODEL_DIR=/models/Qwen3.8-Flash-Next-NVFP4 ./serve-nvfp4-ram.sh
#
# Same image and checkpoint as ../nvfp4-nvme/serve-nvfp4-nvme.sh, but the 47.7 GiB N-gram (PLE)
...
docker run -d --name "$NAME" --restart unless-stopped --gpus '"device=0"' \
  ...
  --ulimit memlock=-1 \
  --label "rtxpro6000-llm.profile=qwen3.8-flash-next-sglang-nvfp4-ram" \
